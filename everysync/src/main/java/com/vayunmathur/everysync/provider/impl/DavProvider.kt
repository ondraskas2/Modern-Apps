...
abstract class DavProvider(
    ...
    override suspend fun sync(context: Context, config: AccountConfig, direction: SyncDirection) {
        ...
        if (DataType.CONTACTS in capabilities && DataType.CONTACTS in config.enabledTypes) {
            syncContacts(context, account, client, contactsBaseUrl(config, creds), direction)
        }
        if (DataType.CALENDAR in capabilities && DataType.CALENDAR in config.enabledTypes) {
            syncCalendars(context, account, client, calendarBaseUrl(config, creds), direction)
        }
    }

    private suspend fun syncContacts(context: Context, account: String, client: DavClient, baseUrl: String, direction: SyncDirection) {
    ...
    private suspend fun syncCalendars(context: Context, account: String, client: DavClient, baseUrl: String, direction: SyncDirection) {
