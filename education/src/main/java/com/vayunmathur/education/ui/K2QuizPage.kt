...
/**
 * K-2 quiz: tap-the-tile multiple choice only. The prompt is narrated on show
 * (tap the speaker to hear again); a correct tap celebrates and advances, a
 * wrong tap gives a gentle "try again" with no penalty and no score.
 */
...
fun K2QuizPage(backStack: NavBackStack<Route>, viewModel: EducationViewModel, exerciseId: String) {
    ...
    val questions = remember(exerciseId) {
        content.exercise(exerciseId)
            ?.let { content.questionsFor(it).filterIsInstance<MultipleChoiceQuestion>() }
            ?: emptyList()
    }
    ...
    Scaffold(
        ...
        LaunchedEffect(question.id) {
            wrongIndex = -1
            narrator?.speak(question.prompt.text)
        }
        ...
        fun choose(i: Int) {
            ...
            if (i == question.correctIndex) {
                answers[question.id] = ChoiceAnswer(i)
                narrator?.speak(praise())
                celebrating = true
            } else {
                wrongIndex = i
                narrator?.speak("Try again!")
            }
        }
        ...
        Box(
            ...
            Column(
                ...
                Row(verticalAlignment = Alignment.CenterVertically) {
                    ...
                    IconButton(onClick = { narrator?.speak(question.prompt.text) }) {
                        Text("🔊", fontSize = 28.sp)
                    }
                }
